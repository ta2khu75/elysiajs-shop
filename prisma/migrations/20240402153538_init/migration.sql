...
CREATE TABLE `users` (
    ...
    `name` VARCHAR(191) NOT NULL,
    `clock` BOOLEAN NOT NULL DEFAULT false,
...
CREATE TABLE `number_of_product` (
    ...
    UNIQUE INDEX `number_of_product_productDetailsId_key`(`productDetailsId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `number_of_product` ADD CONSTRAINT `number_of_product_productDetailsId_fkey` FOREIGN KEY (`productDetailsId`) REFERENCES `product_details`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
